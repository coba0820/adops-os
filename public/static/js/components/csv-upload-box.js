 ...
 * @param {string} options.fileType upload_history に記録するタイプ('ad_media'等)
...
export function mountCsvUploadBox(rootEl, options) {
  const { id, fileType, showMediaSelect, mediaList = [] } = options

  const mediaSelectHtml = showMediaSelect
    ? `
      <div class="form-row">
        <label class="form-label" for="${id}-media-select">媒体を選択</label>
        <select id="${id}-media-select" class="form-select">
          <option value="">-- 媒体を選択してください --</option>
          ${mediaList.map((m) => `<option value="${m.id}">${m.media_name}</option>`).join('')}
        </select>
      </div>
    `
    : ''
    ...
    reader.onload = async (event) => {
      ...
      try {
        await axios.post('/api/upload', {
          ...
          file_name: file.name,
          row_count: Math.max(0, rows.length - 1), // ヘッダー行を除く
        })
        showToast(`「${file.name}」を取込みました（${Math.max(0, rows.length - 1)}件）`, 'success')
      } catch (err) {
        console.error(err)
        showToast('アップロード履歴の記録に失敗しました', 'error')
      }
    }
